        ...
        <div className="relative inline-flex flex-wrap justify-center items-center gap-4 mx-auto">
          {[30, 60, 90, 120, 200, 500, 1000].map((wpm, index) => (
            <button
              key={index}
              type="button"
              className={`relative flex flex-col items-center w-16 py-3 border-2 rounded-md ${wpm === state.targetWPM ? 'border-green-500 text-green-400 bg-green-950 after:content-[""] after:absolute after:bottom-0 after:-mb-3 after:border-green-500 after:w-1/3 after:border-b-4 after:rounded-full' : 'border-neutral-700 text-neutral-400'} hover:border-green-500 hover:text-green-400`}
          ...
          {[1, 3, 5, 10, 25].map((streak, index) => (
            <button
              key={index}
              type="button"
              className={`relative flex flex-col items-center w-16 py-3 border-2 rounded-md ${streak === state.targetStreak ? 'border-sky-400 text-sky-400 bg-sky-950 after:content-[""] after:absolute after:bottom-0 after:-mb-3 after:border-sky-500 after:w-1/3 after:border-b-4 after:rounded-full' : 'border-neutral-700 text-neutral-400'} hover:border-sky-400 hover:text-sky-400`}
          ...
          <button
            type="button"
            className="flex flex-col items-center w-16 py-3 border-2 border-neutral-700 text-neutral-400 hover:border-green-500 hover:text-green-400 rounded-md"
            onClick={handleSave}
          >
          ...
          <button
            type="button"
            className="flex flex-col items-center w-16 py-3 border-2 border-neutral-700 text-neutral-400 hover:border-red-500 hover:text-red-400 rounded-md"
            onClick={handleReset}
          >
      ...
      {state.showInstructions && (
        <div className="fixed inset-0 flex justify-center items-center bg-neutral-900 bg-opacity-60 backdrop-blur-md">
          <div className="text-center">
            ...
            <h1 className="mt-6 text-7xl font-bold text-green-500">Burst<span className="text-white">Type</span></h1>
            ...
            <button className="mt-16 bg-green-600 hover:bg-green-500 text-black font-bold px-5 py-3 rounded-md" type="button" onClick={() => dispatch({type: 'TOGGLE_INSTRUCTIONS'})}>Get Started!</button>
          </div>
        </div>
      )}
    </main>
